...
import { 
  ...
  checkEmailStatus, 
  listActiveAccesses 
} from './src/api/webhooks/kirvano.js'
...
app.get('/api/kirvano/status/:email', async (req, res) => {
  try {
    import { email } = req.params
    
    // Importar função de verificação
    import { checkPurchaseAccess } = require('./src/lib/kirvano.js')
...
app.post('/api/kirvano/simulate-webhook', async (req, res) => {
  try {
    import { email, product_id, purchase_id, amount } = req.body
    ...
    // Processar webhook
    import { processPurchaseWebhook } = require('./src/lib/kirvano.js')
...
app.get('/api/kirvano/config', (req, res) => {
  try {
    import { kirvanoClient } = require('./src/lib/kirvano.js')
...
app.post('/api/kirvano/clear-cache', async (req, res) => {
  try {
    import { clearLocalPurchaseAccess } = require('./src/lib/kirvano.js')
...
app.post('/api/activities/favorites', async (req, res) => {
  try {
    import { childId, activityId } = req.body
...
app.get('/api/analytics/usage', async (req, res) => {
  try {
    import { startDate, endDate } = req.query
